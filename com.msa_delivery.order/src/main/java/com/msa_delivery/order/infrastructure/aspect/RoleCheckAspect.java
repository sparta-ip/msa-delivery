...
public class RoleCheckAspect {
    ...
    // 주문 수정 권한 검사
    private void checkUserRoleForUpdate(String user_id, String username, String role,
        UUID order_id) {
        Long userId = Long.valueOf(user_id); // String -> Long 변환

        // DELIVERY_MANAGER 또는 COMPANY_MANAGER 권한이 있는 경우 수정 불가
        if ("DELIVERY_MANAGER".equals(role) || "COMPANY_MANAGER".equals(role)) {
            log.warn(
                "DELIVERY_MANAGER, COMPANY_MANAGER role cannot update orders. Access denied for user_id: {}, role: {}",
                userId, role);
            throw new AccessDeniedException("수정 권한이 없습니다.");
        }

        checkUserRoleForOrder(userId, username, role, order_id);
    }

    // 주문 삭제 권한 검사
    private void checkUserRoleForDelete(String user_id, String username, String role,
        UUID order_id) {
        Long userId = Long.valueOf(user_id); // String -> Long 변환
        ...
        if ("DELIVERY_MANAGER".equals(role) || "COMPANY_MANAGER".equals(role)) {
            log.warn(
                "DELIVERY_MANAGER, COMPANY_MANAGER role cannot delete orders. Access denied for user_id: {}, role: {}",
                userId, role);
    ...
    private void checkUserRoleForOrder(Long user_id, String username, String role, UUID order_id) {
        log.info("AOP Role Check - user_id: {}, username: {}, role: {}, orderId: {}", user_id,
            username, role, order_id);
        ...
        if ("HUB_MANAGER".equals(role)) {
            ...
            log.info("Order ID: {} is linked to the following Hub IDs: {}", order_id,
                hubIdsOfOrder);
            ...
            boolean hasAccess = hubIdsOfOrder.stream().anyMatch(hub_id -> {
                try {
                    ...
                    log.info("Hub ID: {}, Hub Manager ID: {}, Current User ID: {}", hub_id,
                        hubManagerId, user_id);
                    return hubManagerId != null && hubManagerId.equals(user_id);
                } catch (Exception e) {
                    log.error("Error occurred while retrieving Hub ID: {} - {}", hub_id,
                        e.getMessage());
                    return false;
                }
            });
        ...
        if ("COMPANY_MANAGER".equals(role)) {
            ...
            log.info("Order ID: {} is linked to the following companyMangersId: {}", order_id,
                companyMangersId);

            // 업체 담당자 ID 와 일치하는 user_id 가 있는지 확인
            if (!companyMangersId.contains(user_id)) {
                log.warn(
                    "Access denied for user_id: {}, role: {}, order_id: {}. User is not authorized to access this order.",
                    user_id, role, order_id);
        ...
        if ("DELIVERY_MANAGER".equals(role)) {
            ...
            log.info("Order ID: {} is linked to the following deliveryManagerIds: {}", order_id,
                deliveryManagerIds);
    ...
    public void checkUserRoleForUpdate(Object orderRequestDto, String user_id, String username,
        String role, UUID order_id) {
    ...
    public void checkUserRoleForDelete(Object orderRequestDto, String user_id, String username,
        String role, UUID order_id) {
    ...
    public void checkUserRoleForGet(String user_id, String username, String role, UUID order_id) {
        checkUserRoleForOrder(Long.valueOf(user_id), username, role,
            order_id); // user_id를 Long으로 변환하여 전달
    }
